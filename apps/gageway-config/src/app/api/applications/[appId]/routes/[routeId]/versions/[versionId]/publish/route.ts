import { NextRequest, NextResponse } from 'next/server'
import { prisma } from '@/db/prisma'
import { auth } from '@/auth'

// 发布指定版本
export async function POST(
  request: NextRequest,
  { params }: { params: { appId: string; routeId: string; versionId: string } }
) {
  try {
    const session = await auth()
    if (!session?.user?.id) {
      return NextResponse.json(
        { success: false, message: '未授权访问' },
        { status: 401 }
      )
    }

    const { appId, routeId, versionId } = await params;

    // 检查用户权限
    const permission = await prisma.appPermission.findFirst({
      where: {
        userId: session.user.id,
        applicationId: appId,
        permissions: { has: 'DEPLOY' }
      }
    })

    if (!permission) {
      return NextResponse.json(
        { success: false, message: '无权限发布路由' },
        { status: 403 }
      )
    }

    // 检查路由和版本是否存在
    const route = await prisma.route.findFirst({
      where: {
        id: routeId,
        applicationId: appId
      },
      include: {
        publishedVersion: true
      }
    })

    if (!route) {
      return NextResponse.json(
        { success: false, message: '路由不存在' },
        { status: 404 }
      )
    }

    const version = await prisma.routeVersion.findFirst({
      where: {
        id: versionId,
        routeId: routeId
      },
      include: {
        redirectConfig: true,
        ossConfig: true
      }
    })

    if (!version) {
      return NextResponse.json(
        { success: false, message: '版本不存在' },
        { status: 404 }
      )
    }

    // 执行发布操作
    const result = await prisma.$transaction(async (tx) => {
      // 如果之前有发布版本，先将其状态改为 INACTIVE
      if (route.publishedVersion && route.publishedVersion.id !== versionId) {
        await tx.routeVersion.update({
          where: { id: route.publishedVersion.id },
          data: {
            status: 'INACTIVE',
            publishedAt: null,
            publishedBy: null
          }
        })
      }

      // 发布指定版本
      const publishedVersion = await tx.routeVersion.update({
        where: { id: versionId },
        data: {
          status: 'ACTIVE',
          publishedAt: new Date(),
          publishedBy: session.user!.id
        },
        include: {
          redirectConfig: true,
          ossConfig: true
        }
      })

      // 更新路由的发布版本指针
      await tx.route.update({
        where: { id: routeId },
        data: {
          publishedVersionId: versionId
        }
      })

      // 创建版本记录
      await tx.configVersion.create({
        data: {
          applicationId: appId,
          routeId: routeId,
          versionId: versionId,
          version: version.version,
          action: 'DEPLOY',
          description: `发布版本 ${version.version}`,
          configData: {
            version: version.version,
            configType: version.configType,
            redirectConfig: version.redirectConfig,
            ossConfig: version.ossConfig
          },
          createdBy: session.user!.id
        }
      })

      // 记录操作日志
      await tx.auditLog.create({
        data: {
          userId: session.user!.id,
          applicationId: appId,
          action: 'PUBLISH_VERSION',
          resource: 'route',
          resourceId: routeId,
          details: {
            routeName: route.name,
            version: version.version,
            versionId: versionId,
            publishedAt: new Date().toISOString()
          }
        }
      })

      return publishedVersion
    })

    return NextResponse.json({
      success: true,
      data: result,
      message: `版本 ${version.version} 发布成功`
    })

  } catch (error) {
    console.error('发布版本失败:', error)
    return NextResponse.json(
      { success: false, message: '发布版本失败' },
      { status: 500 }
    )
  }
} 